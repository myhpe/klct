...
def menu_check_ldap_connection_basic(screen):
    ...
    if valid_ip_addr[0] == 0:
        screen.addstr(screen_dims[0]/2 - 4, screen_dims[1]/2 - 26, "No valid IP found. Please complete the previous step", curses.A_BOLD | curses.color_pair(3))
        screen.addstr(screen_dims[0]/2 - 3, screen_dims[1]/2 - 23, "Press p to go to previous step, or 'm' for menu", curses.color_pair(5))
        ...
        temp_str = my_raw_input(screen, screen_dims[0]/2 - 3, screen_dims[1]/2 - 22, "Please enter the port number. Default is 389.")
        ...
        screen.addstr(screen_dims[0]/2, screen_dims[1]/2 - 18, "Attempting to connect to LDAP server", curses.color_pair(5))
        screen.refresh()
        triple = configTool.connect_LDAP_server_basic(host_ip, port_numb)
        if triple[0] == 1:
            screen.addstr(screen_dims[0]/2 + 1, screen_dims[1]/2 - len(triple[1])/2, triple[1], curses.color_pair(6) | curses.A_BOLD)
            ...
            screen.addstr(screen_dims[0]/2 + 1, screen_dims[1]/2 - len(triple[1])/2, triple[1], curses.color_pair(3) | curses.A_BOLD)
